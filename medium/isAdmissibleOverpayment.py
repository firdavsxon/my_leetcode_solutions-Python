...
def isAdmissibleOverpayment(prices, notes, x):
	...
	for i in notes:
		if 'higher' in i:
			notes_array.append((float(i.split('%')[0]), 1))
		elif 'lower' in i:
			notes_array.append((float(i.split('%')[0]), -1))
		else:
			notes_array.append((0, 0))

	for a, b in zip(prices, notes_array):
		if b[1] == -1:
			original = (a * 100) / (100 - b[0])
		elif b[1] == 1:
			original = (a * 100) / (100 + b[0])
		else:
			original = a
...
print(isAdmissibleOverpayment(prices, notes, x=5))
